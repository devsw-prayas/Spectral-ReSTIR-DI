...
Coverage Lemma: under static geometry (reprojection Jacobian ≡ 1, the
locked v1 scope -- no camera/object motion), combining a current-frame
...
`p_hat_i,gen(x)=0`. For a genuine scene change (disocclusion) this collapses
to one cheap, exact, threshold-free runtime check: `wsum_gen == 0` on the
history reservoir must zero its effective confidence in the combine
**denominator**, not just skip its own candidate -- excluding it from
`combine_reservoirs` entirely (M=0) does both at once, since
`combine_reservoirs` already drops any reservoir with `M == 0`.

This module adds only that gate and the M-clamp-on-input plumbing on top of
module 4 -- current and history share the same pixel/vertex domain under
static geometry, so the shift maps between them are always identity, and
`combine_reservoirs`'s own `target_pdf_fn`/`contribution_weight` split
(eval-time target vs. each reservoir's own stored `p_hat_gen`) already
implements the gen/eval distinction the Coverage Lemma needs -- no new
combine arithmetic, just correct wiring.

**Flagged gap, not solved here:** no volumetric-temporal analog of the
Coverage Lemma exists. The volumetric case needs a genuine *practical*
variance cutoff instead: the transmittance-based gen density is never
exactly zero at finite optical depth, so the exact `wsum_gen==0` check never
fires the way it does in this surface case -- out of scope for this module.
...
class HistoryBuffer:
    """Per-pixel temporal reservoir history, keyed by pixel index.

    Locked v1 scope is static geometry only -- `reproject` is always
    identity (the reprojection-Jacobian-≡1 precondition holds
    ...
    def reproject(self, motion_vectors=None) -> "HistoryBuffer":
        ...
        `motion_vectors` only to keep this call site ready for a future
        rigid-motion extension (out of scope for this paper) -- the argument
        is never read.
        """
        return self
...
def temporal_combine(current_reservoir, history_reservoir, target_pdf_fn, rng, m_cap=None, wsum_gen_gate=0.0):
    """Combine `current_reservoir` and `history_reservoir` for one pixel.
    ...
    the history reservoir is excluded from the combine entirely (both its
    own candidate and its confidence in the MIS denominator), rather than
    contributing a phantom low-confidence term -- same drop-not-zero-fill
    discipline as `mis_combine`'s nonexistent-shift case.

    M-clamping: `m_cap` clamps the input `M` of every source reservoir
    before merging (via `combine_reservoirs`'s own `m_cap`) -- clamping the
    combined output's `M` afterward bounds nothing, since
